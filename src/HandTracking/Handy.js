...
Object.assign(Handy.protos, {
	...
	distanceBetweenJoints: function(jointNameA, jointNameB) {
		...
		const hand = this,
			[jointA, jointB] = [jointNameA, jointNameB].map(function(name) {
				return hand.joints[
	...
	reportDigits: function() {
		...
		Handy.digitNames.forEach(function(digitName) {
			const proximalName =
				...
				tipName =
					digitName === "thumb" ? "thumb-tip" : digitName + "-finger-tip",
				distance = hand.distanceBetweenJoints(proximalName, tipName),
	...
	readLivePoseData: function() {
		const hand = this,
			...
			preparePosition = function(joint) {
				const jointMatrix = joint.matrix.clone().premultiply(
			...
			headPosition =
				wrist !== undefined && !wrist.position.equals(Handy.VECTOR3_ZERO)
					...
					: null,
			headRotation =
				headPosition === null ? null : hand.camera.quaternion.toArray();
	...
	showPoseByName: function(poseName, matrix) {
		const hand = this,
			pose = Handy.poses[hand.handedness].find(function(pose) {
	...
	search: function() {
		...
		for (let i = hand.searchPoseIndex; i < poses.length; i++) {
			...
			if (i === 0) {
				...
				if (
					hand.livePoseData.jointPositions.length === 0 ||
			...
			if (method === "digitTipPositions") {
				hand.searchResultsBuffer.push({
					pose,
					distance: pose.digitTipPositions.reduce(function(
						distance,
						digitTipPosition,
						i
					) {
						if (
							digitTipPosition.length !== undefined &&
							...
							distance += //Math.sqrt(
								Math.pow(
									digitTipPosition[0] -
										hand.livePoseData.digitTipPositions[i][0],
									2
								) +
								Math.pow(
									digitTipPosition[1] -
										hand.livePoseData.digitTipPositions[i][1],
									2
								) +
								Math.pow(
									digitTipPosition[2] -
										hand.livePoseData.digitTipPositions[i][2],
									2
								);
							//)
						}
				...
				hand.searchResultsBuffer.push({
					pose,
					distance: pose.jointPositions.reduce(function(
						distance,
						jointPosition,
						i
					) {
						if (
							jointPosition.length !== undefined &&
							...
							distance += //Math.sqrt(
								Math.pow(
									jointPosition[0] - hand.livePoseData.jointPositions[i][0],
									2
								) +
								Math.pow(
									jointPosition[1] - hand.livePoseData.jointPositions[i][1],
									2
								) +
								Math.pow(
									jointPosition[2] - hand.livePoseData.jointPositions[i][2],
									2
								);
							//)
						}
						...
						hand.lastSearchResult.pose.names.forEach(function(poseName) {
							hand.dispatchEvent({
								...
								message:
									...
									hand.lastSearchResult.distance +
									"mm.",
							});
						});
			...
			//  check if this search is taking too long per update() loop.
				...
				if (
					window.performance.now() - hand.searchLoopBeganAt >
	...
	isPose: function(poseName, threshold) {
		...
		if (typeof threshold === "number") {
			const result = hand.searchResults.find(function(result) {
				return (
					result.distance <= threshold && result.pose.names.includes(poseName)
				);
			});
	...
	comparePoses: function(poseAName, poseBName) {
		const hand = this,
			...
			poseA = posesList.find(function(pose) {
		...
		let poseDistanceAbs = 0,
			poseDistanceSqr = 0;

		for (let i = 0; i < poseA.positions.length; i++) {
			const positionA = poseA.positions[i],
	...
	update: function(callback) {
		...
		if (
			...
			if (hand.searchResults.length && hand.searchResults[0].pose) {
				displayString +=
					"\n" +
					hand.searchResults[0].pose.names.reduce(function(names, name, i) {
						if (i) names += ", ";
						return (names += name);
					}, "");
				displayString +=
